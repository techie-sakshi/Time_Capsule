...
export default function Dashboard() {
  ...
  const sendUnlockEmail = (capsule) => {
    emailjs.send(
      process.env.NEXT_PUBLIC_EMAILJS_SERVICE_ID,
      process.env.NEXT_PUBLIC_EMAILJS_TEMPLATE_ID,
      {
        user_name: user.displayName || user.email,
        // capsule_title: capsule.message,
        unlock_date: capsule.unlockDate.toLocaleString(),
        to_email: user.email,
      },
  ...
  useEffect(() => {
    capsules.forEach((cap) => {
      if (isUnlocked(cap) && !cap.reminderSent) {
        sendUnlockEmail(cap);
        const capsuleRef = doc(db, "capsules", cap.id);
        updateDoc(capsuleRef, { reminderSent: true });
      }
    });
  }, [capsules]);
